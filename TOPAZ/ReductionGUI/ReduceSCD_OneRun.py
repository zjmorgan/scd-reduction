...
sys.path.insert(0,"/opt/mantid51/lib")
...
use__md_gaussian          = params_dictionary.get("use__md_gaussian",'False')
n_bad_edge_pixels         = params_dictionary.get("n_bad_edge_pixels",'18')
...
BN_aperture_size           = params_dictionary.get("BN_aperture_size", '2')

split_threshold            = params_dictionary[ "split_threshold" ]

Qmax                       = params_dictionary.get('Qmax', "15")

maxQ ='%s,%s,%s'%(Qmax,Qmax,Qmax)
minQ ='-%s,-%s,-%s'%(Qmax,Qmax,Qmax)

satellite_peak_size              = params_dictionary.get("satellite_peak_size",'0.09')
satellite_background_inner_size  = params_dictionary.get("satellite_background_inner_size",'0.091')
satellite_background_outer_size  = params_dictionary.get("satellite_background_outer_size",'0.10')
satellite_region_radius          = params_dictionary.get("satellite_region_radius",'0.10')
...
cut_off_sig                      = params_dictionary.get("cut_off_sig", '3')
num_sigmas                       = params_dictionary.get("num_sigmas", '3')

remove_Al_peaks                  = params_dictionary.get("remove_Al_peaks",'False')
filter_peaks                     = params_dictionary.get("filter_peaks",'False')
#
...
#
run_niggli_matrix_file = output_directory + "/" + str(run)+ "_Niggli.mat"
run_niggli_integrate_file = output_directory + "/" + str(run) + "_Niggli.integrate"

#
ipts_folder_name=GetIPTS(run)
#
# Load the run data and find the total monitor counts

if subtract_bkg is not True:
  print("\nProcessing File: " + full_name + " ......\n")
  ...
  if os.path.exists(bkg_subtracted_event_fname):
    ...
    temp=0.98*bkg*(proton_charge/bkg_proton_charge) 
    event_ws=Minus(LHSWorkspace=event_ws, RHSWorkspace=temp)
    SaveNexus(event_ws,bkg_subtracted_event_fname)
...
peaks_ws = FindPeaksMD( MDEW, MaxPeaks=num_peaks_to_find,DensityThresholdFactor='5', 
                        PeakDistanceThreshold=distance_threshold, EdgePixels=n_bad_edge_pixels)
#
#Remove peaks on Al ring
if remove_Al_peaks:
  al_peaks=[0.74, 0.82, 0.91, 1.02, 1.14,1.22,1.43, 2.03, 2.33, 2.35, 2.37, 2.73]
  peaks_on_Al_ring = []
  print('Remove peaks on aluminum ring')
  for j in range(0,len(al_peaks)):
    d_low =al_peaks[j]-0.008
    d_uper=al_peaks[j]+0.008
    for i in range(peaks_ws.getNumberPeaks()):
      pi=peaks_ws.getPeak(i)
      if (pi.getDSpacing()>=round(d_low,3) and pi.getDSpacing()<=round(d_uper,3)):
        peaks_on_Al_ring.append(i)
  DeleteTableRows(TableWorkspace=peaks_ws,Rows=peaks_on_Al_ring)
#
...
if read_UB:
  ...
  if os.path.exists(ubpath +'/%s_Niggli.mat'%(run)):
    ...
    IndexPeaks( PeaksWorkspace=peaks_ws, CommonUBForAll=False, Tolerance=tolerance,ToleranceForSatellite=tolerance_satellite)
    FindUBUsingIndexedPeaks(PeaksWorkspace=peaks_ws, Tolerance=tolerance,ToleranceForSatellite=tolerance_satellite)
...
if save_mod_info is True: 
  IndexPeaks(PeaksWorkspace=peaks_ws, Tolerance=tolerance, ToleranceForSatellite=tolerance_satellite, 
                  ...
                  MaxOrder = max_order, CrossTerms=cross_terms, CommonUBForAll =False)
  FindUBUsingIndexedPeaks(PeaksWorkspace=peaks_ws, Tolerance=tolerance,ToleranceForSatellite=tolerance_satellite)
...
if peaks_ws.sample().hasOrientedLattice():
  ...
  max_order = peaks_ws.sample().getOrientedLattice().getMaxOrder()
else:
  max_order = 0

print('hkl range :  %s --> %s'%(min_hkl,max_hkl))
print('Max order for Satellite peak %d'%(max_order))
#
...
if integrate_predicted_peaks:
  ...
  if max_order >0:
    peaks_ws = PredictSatellitePeaks(Peaks=peaks_ws, MaxOrder=max_order, GetModVectorsFromUB=True, IncludeAllPeaksInRange=True, 
                 WavelengthMin=min_pred_wl, WavelengthMax=max_pred_wl,
                 MinDSpacing=min_pred_dspacing, MaxDSpacing=max_pred_dspacing)

  #Remove peaks on detector edge
  peaks_on_edge=[]
  print('Remove peaks %s pixels on detector edge'%(n_bad_edge_pixels))
  edge_low =int(n_bad_edge_pixels)
  edge_uper=255-int(n_bad_edge_pixels)
  for i in range(peaks_ws.getNumberPeaks()):
    pi=peaks_ws.getPeak(i)
    if pi.getRow()<edge_low or pi.getRow()>edge_uper or pi.getCol()<edge_low or pi.getCol()>edge_uper:
  ...
  #peaks_ws = CentroidPeaks(InPeaksWorkspace=peaks_ws, InputWorkspace=event_ws, PeakRadius=4, EdgePixels=18)
  ...
  FindUBUsingIndexedPeaks(PeaksWorkspace=peaks_ws, Tolerance=tolerance, ToleranceForSatellite=tolerance_satellite)
  IndexPeaks(PeaksWorkspace=peaks_ws, Tolerance=tolerance, RoundHKLs=False,ToleranceForSatellite=tolerance_satellite)
  FindUBUsingIndexedPeaks(PeaksWorkspace=peaks_ws, Tolerance=tolerance,ToleranceForSatellite=tolerance_satellite)
...
#
# Integrate found or predicted peaks in HKL space using spheres or Gaussian ellipsoid, and save 
...
# Note: Ellipsoid peak integration in MD is very slow !!!

  if integrate_in_HKL_space:
    CopySample(InputWorkspace=peaks_ws, OutputWorkspace=event_ws, 
                   CopyName=False, CopyMaterial=False, CopyEnvironment=False, CopyShape=False)
    MDEW = ConvertToMD(InputWorkspace=event_ws, QDimensions='Q3D', dEAnalysisMode='Elastic', 
                   Q3DFrames='HKL', LorentzCorrection='0', 
                   QConversionScales='HKL', Uproj='1,0,0', Vproj='0,1,0', Wproj='0,0,1', 
                   AbsMinQ=0.35, MinValues=min_hkl, MaxValues=max_hkl,
                   SplitInto='2', SplitThreshold=split_threshold,MaxRecursionDepth='15')

    peaks_ws = IntegratePeaksMD(InputWorkspace=MDEW, 
                   PeakRadius=peak_radius, BackgroundInnerRadius=bkg_inner_radius, BackgroundOuterRadius=bkg_outer_radius,
                   PeaksWorkspace=peaks_ws, Ellipsoid=use__md_gaussian, 
                   CorrectIfOnEdge=integrate_if_edge_peak, UseOnePercentBackgroundCorrection=False)
  else:
    ...
    peaks_ws = IntegratePeaksMD( InputWorkspace=MDEW, PeakRadius=peak_radius,
                   ...
                   PeaksWorkspace=peaks_ws, Ellipsoid=use__md_gaussian, 
                   CorrectIfOnEdge=integrate_if_edge_peak, 
  ...
  peaks_ws= IntegrateEllipsoids( InputWorkspace=event_ws, 
                                 ...
                                 CutoffIsigI=cut_off_sig,  
                                 NumSigmas=num_sigmas,
                                 PeakSize = peak_radius,
                                 BackgroundOuterSize = bkg_outer_radius,
                                 BackgroundInnerSize = bkg_inner_radius,
                                 # SatellitePeakSize=satellite_peak_size,
                                 # SatelliteBackgroundInnerSize=satellite_background_inner_size,
                                 # SatelliteBackgroundOuterSize=satellite_background_outer_size,
                                 # SatelliteRegionRadius=satellite_region_radius,
...
# result.
#
SaveIsawPeaks( InputWorkspace=peaks_ws, AppendFile=False, Filename=run_niggli_integrate_file, RenumberPeaks=True )
