...
for r_num in run_nums:
  ...
  cmd = 'python3 ' + reduce_one_run_script + ' ' + config_file_name + ' ' + str(r_num)
...
#Index peaks using UB from UB of initial orientation run/or combined runs from first iteration of crystal orientation refinement
  if maxOrder > 0:
    IndexPeaks(PeaksWorkspace=peaks_ws, Tolerance=tolerance, CommonUBForAll=False, RoundHKLs=False)
  elif read_UB:
...
end_time = time.time()
print('\nTotal time:   ' + str(end_time - start_time) + ' sec')
print('\nConfig file used for data reduction : ' + config_file_name) 
print('\nScript file:  ' + reduce_one_run_script + '\n')

print("\n**************************************************************************************")
print("************************* Part 4 ANVRED Reduction **************************************")
print("**************************************************************************************\n")
